...
public interface KgerListMapper {
	...
	@Select("SELECT * FROM (SELECT t.*, ROWNUM AS rnum FROM (SELECT * FROM tb_kindergartener) t WHERE ROWNUM <= #{offset} + #{pageSize} AND CLASS_IDX = #{class_idx} order by kger_name) WHERE rnum > #{offset} AND rnum <= #{offset} + #{pageSize}")
